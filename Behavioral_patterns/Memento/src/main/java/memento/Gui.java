...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        Label labelRedo = new Label("Press Ctrl-Y to redo the last undone action.");
        ...
        // create a VBox that contains the HBox and the CheckBox,label, labelRedo, historyButton
        VBox vBox = new VBox(hBox, checkBox, label, labelRedo, historyButton);
        vBox.setSpacing(10);
        ...
        scene.setOnKeyPressed(event -> {
            if (event.isControlDown() && event.getCode() == KeyCode.Z) {
                // Ctrl-Z: undo
                System.out.println("Undo key Z combination pressed");
                controller.undo();
            }
            if (event.isControlDown() && event.getCode() == KeyCode.Y) {
                // Ctrl-Y: redo
                System.out.println("Redo key Y combination pressed");
                controller.redo();
            }
        });
